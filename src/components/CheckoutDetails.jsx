...
const CheckoutDetails = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-primary font-sans text-white overflow-x-hidden relative">
      ...
      {/* 1. HEADER */}
      <header className="p-6 md:p-10 flex justify-center w-full bg-primary">
      ...
      <main className="flex-grow flex justify-center items-start px-4 py-10 bg-white">
        <div className="bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.1)] w-full max-w-5xl flex flex-col md:flex-row overflow-hidden min-h-[550px] text-black border border-gray-100">
          
          {/* Blue Section: Your Order */}
          <div className="bg-[#635bff] text-white p-10 md:w-[380px] flex flex-col">
            <h3 className="text-[11px] font-bold opacity-70 uppercase tracking-[0.2em] mb-8">Your Order</h3>
            <div className="flex-grow space-y-8">
              <div>
                <h2 className="text-2xl font-black leading-tight mb-1">{title}</h2>
                <p className="text-sm opacity-80 truncate">{email}</p>
              </div>
              
              <div>
                <p className="text-[10px] opacity-60 uppercase font-black mb-1 tracking-widest">Amount</p>
                <p className="text-xl font-bold">${totalAmount} USD</p>
              </div>

              <div>
                <p className="text-[10px] opacity-60 uppercase font-black mb-1 tracking-widest">Order ID</p>
                <p className="text-sm font-mono opacity-90">{id || 'EsctzynWagcStzJRb'}</p>
              </div>

              <div>
                <p className="text-[10px] opacity-60 uppercase font-black mb-1 tracking-widest">Date</p>
                <p className="text-sm font-medium">Today at 11:34 AM</p>
              </div>
            </div>
            <button className="mt-12 w-full border border-white/30 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">
              Contact Support
            </button>
          </div>

          {/* White Section: Payment Selection */}
          <div className="flex-grow p-10 md:p-16 flex flex-col items-center justify-center bg-[#fcfdfe]">
            <h2 className="text-3xl font-black text-gray-800 mb-10 tracking-tighter">Select payment method</h2>
            
            <div className="w-full max-w-md bg-white border-2 border-[#635bff] p-6 rounded-2xl flex items-center gap-5 shadow-sm">
              <div className="w-6 h-6 rounded-full border-2 border-[#635bff] flex items-center justify-center">
            ...
            <div className="w-full max-w-md flex flex-col sm:flex-row justify-between gap-4 mt-20 pt-8 border-t border-gray-100">
              <button onClick={() => navigate('/')} className="px-8 py-3 bg-gray-100 rounded-xl text-[11px] font-black text-gray-500 uppercase hover:bg-gray-200">
                Back to shop
              </button>
              <button onClick={() => setModalStep('currency')} className="px-8 py-3 bg-[#635bff] text-white rounded-xl text-[11px] font-black uppercase flex items-center gap-2 hover:opacity-90 transition-all">
      ...
      {!modalStep && (
        <div className="fixed right-6 bottom-6 flex flex-col gap-3 items-center z-[150]">
          {showMenu && (
            <div className="flex flex-col gap-3 animate-in slide-in-from-bottom-5 duration-300">
              <button onClick={() => setShowMenu(false)} className="bg-gray-800 p-3 rounded-full shadow-lg text-white">
                <ChevronDown size={22}/>
              </button>
              <button className="bg-[#41b1e6] p-3.5 rounded-full shadow-xl text-white hover:scale-110 transition-transform">
                <Send size={20}/>
              </button>
              <button className="bg-primary p-3.5 rounded-full shadow-xl text-white hover:scale-110 transition-transform">
                <UserRoundSearch size={20}/>
              </button>
              <button className="bg-[#635bff] p-3.5 rounded-full shadow-xl text-white hover:scale-110 transition-transform">
                <Search size={20}/>
              </button>
            </div>
          )}
          {!showMenu && (
            <button 
              ...
              className="bg-blue-600 p-5 rounded-full shadow-2xl text-white hover:scale-110 transition-transform"
            >
      ...
      {/* FOOTER */}
      <footer className="bg-white py-8 px-10 flex flex-col md:flex-row justify-between items-center text-gray-400 border-t border-gray-50">
        <div className="text-[10px] font-black uppercase tracking-[0.2em]">© PUNCH SHOP 2026</div>
        <div className="flex items-center gap-2 text-yellow-400 mt-4 md:mt-0">
          {[...Array(5)].map((_, i) => <Star key={i} size={14} fill="currentColor" />)}
          <span className="text-[10px] font-black uppercase ml-2 tracking-widest">(331 Excellent Reviews)</span>
        </div>
      </footer>
    </div>
  );
